pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "team7-eurekaserver-kkh"
        IMAGE_TAG = "latest"
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/KimKyungHyunn/eurekaserver_kkh.git'
            }
        }
        ...
        stage('Deployment Image to Update') {
            steps {
                script {
                    ...
                    sh "kubectl set image deployment/eurekaserver-kkh eurekaserver-kkh=${REGISTRY}/${IMAGE_NAME}:${IMAGE_TAG} --namespace=${NAMESPACE}"
                }
            }
        }
    }
